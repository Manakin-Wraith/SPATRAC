...
def save_report_as_pdf(filename, inventory, report_type, start_date, end_date, auth_system):
    ...
    try:
        ...
        if report_type == 'Inventory Summary':
            for item in report_data:
                pdf.cell(0, 8, f"Product Code: {item.get('Product Code', 'N/A')}", ln=True)
                pdf.cell(0, 8, f"Name: {item.get('Name', 'N/A')}", ln=True)
                pdf.cell(0, 8, f"Quantity: {item.get('Quantity', 'N/A')}", ln=True)
...
def save_report_as_csv(filename, inventory, report_type, start_date, end_date, auth_system):
    ...
    try:
        ...
        if report_type == 'Inventory Summary':
            ...
            headers = ['Product Code', 'Description', 'Supplier Batch', 'Sell By Date', 'Received Date', 'Received By', 'Status']
        else:
            raise ValueError(f"Unknown report type: {report_type}")
