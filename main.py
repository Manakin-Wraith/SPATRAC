...
def generate_report_display(report_data, report_type):
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
"""
    ...
    if report_type == 'Inventory Summary':
        ...
        unique_products = len(set(item['Product Code'] for item in report_data))
        total_quantity = sum(float(item['Quantity']) for item in report_data)
...
"""
        ...
        for item in report_data:
            ...
            product_groups[code]['quantity'] += float(item['Quantity'])
...
def generate_inventory_summary(inventory, start_date, end_date, auth_system):
    try:
        ...
        # Format dates as strings in SQLite format
        start_date_str = start_date.strftime('%Y-%m-%d %H:%M:%S') if isinstance(start_date, datetime) else start_date
        end_date_str = end_date.strftime('%Y-%m-%d %H:%M:%S') if isinstance(end_date, datetime) else end_date
        ...
        ''', (department, start_date_str, end_date_str))
        
        inventory_data = cursor.fetchall()
        conn.close()
        
        if not inventory_data:
            return generate_report_display([{
        ...
        return generate_report_display(report, 'Inventory Summary')
...
def show_product_details(product, auth_system):
    ...
    layout = [
        ...
        [sg.Text(f"Current Location: {product.get('Current Location', 'N/A')}")],
        [sg.Text(f"Processing Date: {product.get('Processing Date', 'N/A')}")],
    ...
    try:
        ...
        if report_type == 'Inventory Summary':
            ...
            for item in report_data:
                ...
                pdf.set_font("Arial", "", 12)
                pdf.cell(0, 8, f"• Code: {item['Product Code']}", ln=True)
                pdf.cell(0, 8, f"• Description: {item['Description']}", ln=True)
                pdf.cell(0, 8, f"• Batch: {item['Supplier Batch']}", ln=True)
                pdf.cell(0, 8, f"• Sell By: {item['Sell By Date']}", ln=True)
                
                pdf.ln(5)
                pdf.set_font("Arial", "B", 12)
                pdf.cell(0, 10, "Tracking Information", ln=True)
                pdf.set_font("Arial", "", 12)
                pdf.cell(0, 8, f"• Received: {item['Received Date']}", ln=True)
                pdf.cell(0, 8, f"• Received By: {item['Received By']}", ln=True)
                pdf.cell(0, 8, f"• Status: {item['Status']}", ln=True)
                pdf.ln(10)
